...
class PrabhupadaRAG:
    ...
    def process_query(
            ...
            messages=[
                {
                    ...
                    "content": (
                        ...
                        "- Examples:\n"
                        "  • 'What did Prabhupada say about Krishna in his lectures?' → 'spoken Krishna consciousness lectures'\n"
                        "  • 'Morning walk conversation about devotional service' → 'morning walk devotional service'\n"
                        "  • 'Room conversation on chanting' → 'room conversation chanting Hare Krishna'\n"

                        "✉️ LETTERS CONTENT HANDLING:\n"
                        ...
                        "- Examples:\n"
                        "  • 'Letter to disciple about initiation' → 'letter initiation spiritual master'\n"
                        "  • 'What Prabhupada wrote about temple management' → 'letters temple management devotees'\n"
                        "  • 'Correspondence with Hayagriva about books' → 'letters Hayagriva book distribution'\n"

                        "🔍 CONTENT-SPECIFIC REFINEMENT PRIORITY:\n"
        ...
        # 2️⃣ STAGE 2: Parse query for specific book/chapter/verse requests and special content types
        ...
        if self._detect_spoken_reference(refined_query):
            print("🎤 Detected spoken content request")
        ...
        # Check for letters reference
            ...
            print("✉️ Detected letters content request")
    ...
    def _process_spoken_reference(self, refined_query: str) -> Tuple[str, List[Dict]]:
        ...
        print(f"🎤 Processing spoken content reference")
        ...
        if not os.path.exists(spoken_faiss_path) or not os.path.exists(spoken_metadata_path):
            print(f"⚠️ Missing spoken data files")
            return "", []
        ...
        try:
            with open(spoken_metadata_path, "r") as f:
                ...
                print(f"✅ Loaded spoken metadata: {len(spoken_metadata)} entries")
        except Exception as e:
            print(f"⚠️ Error loading spoken metadata: {str(e)}")
            return "", []
        
        # Load spoken FAISS index
        try:
            spoken_index = faiss.read_index(spoken_faiss_path)
            print(f"✅ Loaded spoken FAISS index")
        except Exception as e:
            print(f"⚠️ Error loading spoken FAISS index: {str(e)}")
            return "", []
        
        # Generate query embedding and search
        query_vector = model.encode([refined_query])
        D, I = spoken_index.search(query_vector, k=1)  # Get top 1 result
        print(f"🔍 Found {len(I[0])} results in spokens")
        ...
        for i, idx in enumerate(I[0]):
            if idx < len(spoken_metadata):
                ...
                print(f"🔍 Processing spoken content {idx}: {extended_text}...")
        ...
        elapsed_time = time.time() - start_time
        print(f"⏱️ Spoken content processing completed in {elapsed_time:.2f} seconds with {len(completed_contexts)} results")
    ...
    def _process_letters_reference(self, refined_query: str) -> Tuple[str, List[Dict]]:
        ...
        print(f"✉️ Processing letters reference")
        ...
        if not os.path.exists(letters_faiss_path) or not os.path.exists(letters_metadata_path):
            print(f"⚠️ Missing letters data files")
            return "", []
        ...
        try:
            with open(letters_metadata_path, "r") as f:
                ...
                print(f"✅ Loaded letters metadata: {len(letters_metadata)} entries")
        except Exception as e:
            print(f"⚠️ Error loading letters metadata: {str(e)}")
            return "", []
        
        # Load letters FAISS index
        try:
            letters_index = faiss.read_index(letters_faiss_path)
            print(f"✅ Loaded letters FAISS index")
        except Exception as e:
            print(f"⚠️ Error loading letters FAISS index: {str(e)}")
            return "", []
        
        # Generate query embedding and search
        query_vector = model.encode([refined_query])
        D, I = letters_index.search(query_vector, k=5)  # Get top 5 results
        print(f"🔍 Found {len(I[0])} results in letters")
        # Process results
        contexts = []
        retrieval_sources = []
        ...
        elapsed_time = time.time() - start_time
        print(f"⏱️ Letters processing completed in {elapsed_time:.2f} seconds with {len(contexts)} results")
  ...
  "role": "system",
  "content": (
    "You are a spiritual editor for formatting transcripts of Srila Prabhupāda’s conversations, lectures, and interviews. "
    ...
    "1️⃣ 🎙️ **SEGMENT BY SPEAKER**:\n"
    "- Clearly identify speakers like Prabhupāda, Devotee, Guest, Press, etc.\n"
    "- Use bold for each speaker name:\n"
    "  **Prabhupāda:**\n"
    "  **Devotee:**\n"
    "- Place each speaker’s sentence on a new line.\n"
    "- Always leave a BLANK LINE between speaker turns.\n\n"

    "2️⃣ 🔡 **HIGHLIGHT SANSKRIT WORDS IN ENGLISH LINES**:\n"
    "- If Sanskrit terms like dharma, ātmā, karma, yajña appear within English lines, make them **bold**.\n"
    "- Do NOT bold general English words or names.\n\n"

    "3️⃣ 📜 **SHLOKAS FORMATTING RULES**:\n"
    ...
    "4️⃣ ✂️ **REMOVE DISTRACTIONS**:\n"
    "- Delete things like [break], [tape ends], [noise], extra stars, symbols, [**], [chanting], [leads kīrtana], etc.\n"
    ...
    "5️⃣ 🪔 **GROUP PARAGRAPHS BY THEMES**:\n"
    "- Organize Prabhupāda’s words into paragraphs with natural breaks every 2–4 lines.\n"
    "- Paragraphs should be split by topic or sub-point.\n\n"

    "6️⃣ 🏷️ **ADD A MEANINGFUL HEADING BEFORE EACH TOPIC**:\n"
    "- Insert a short, clear heading before each block:\n"
    "  Format: `### 📖 Heading Here`\n"
    "- Use relevant emojis to catch the eye.\n"
    "- Keep headings concise and meaningful (3–6 words).\n"
    "- Leave a blank line after the heading.\n\n"

    "7️⃣ 💡 **KEY INSIGHT FOR EACH TOPIC**:\n"
    ...
    "🎯 PURPOSE: Make the result feel like a **published, sacred, spiritual lecture transcript** — suitable for reading, reflection, sharing, and preaching. Every section should be beautiful, clean, and spiritually impactful."
  )
